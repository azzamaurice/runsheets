...
const exportPng = async (): Promise<void> => {
    ...
    const clone = output.value.cloneNode(true) as HTMLElement;
    clone.style.cssText = '';
    clone.style.position = 'fixed';
    clone.style.top = '-9999px';
    clone.style.left = '-9999px';
    clone.style.width = `${output.value.offsetWidth}px`;
    clone.style.height = `${output.value.offsetHeight}px`;
    document.body.appendChild(clone);

    const allOriginal = Array.from(output.value.querySelectorAll('*')) as HTMLElement[];
    const allCloned = Array.from(clone.querySelectorAll('*')) as HTMLElement[];
    const computedBg = window.getComputedStyle(output.value).backgroundColor;
